...
% Programme d'assemblage du système Au = B
% inconnue: u
% FoncD : pointeur sur la fonction de diffusion (FDiff1, FDiff2...)
% FoncF : pointeur sur la fonction terme source (FSource1, FSource2...)
% Assemblage de la matrice et du deuxième membre


% cette trame de fonction est donnée à titre indicatif
% et pour vous aider
...
for i = 1:ntot
    vois = mvois(i, :);
    if Conbord(i,1)>0     %----condition imposée au noeud i
       if Conbord(i,1)==1      % type Dirichlet
          A(i,i)=1;

       elseif vois(1)==0     % type Neumann suivant frontière à droite : du/dx
          v1=mvois(vois(3),3);
          v2=vois(3);
          A(i,v1)=1/(2*pasx);
          A(i,v2)=-4/(2*pasx);
       ...
       elseif vois(3)==0  % neumann à gauche : -du/dx
          v1=mvois(vois(1),1);
          v2=vois(1);
          A(i,v1)=1/(2*pasx);
          A(i,v2)=-4/(2*pasx);
